...
// Walk-ins dürfen einen kleinen Puffer „verdrängen“
...
// Admin-Mailziel robust auflösen
const ADMIN_TO =
...
async function notifyAdmin(subject: string, html: string) {
  if (!ADMIN_TO) return;
  try {
    await sendEmailSMTP(ADMIN_TO, subject, html);
  } catch {}
}
...
// ----------------------------------------------------------------------------
// Öffentliche Config (für Client, falls nötig)
// ----------------------------------------------------------------------------
...
app.post("/api/reservations", async (req, res) => {
  ...
  if (g > MAX_ONLINE_GUESTS) {
    return res.status(400).json({
      error:
        "Online bookings are limited to 10 guests. Please contact us directly.",
    });
  }
...
// ----------------------------------------------------------------------------
// Cancel (idempotent) -> schickt Gast/ Admin Mails + zeigt HTML
// ----------------------------------------------------------------------------
app.get("/cancel/:token", async (req, res) => {
  ...
  if (!wasCanceled) {
    ...
    if (r.email && r.email !== "walkin@noxama.local") {
      const guestHtml = canceledGuestHtml(
        r.firstName,
        r.name,
        r.date,
        r.time,
...
// ----------------------------------------------------------------------------
/** Export Excel (daily/weekly/monthly/yearly) */
// ----------------------------------------------------------------------------
app.get("/api/export", async (req, res) => {
  try {
    ...
    // progressive visit count per email innerhalb des Zeitraums
    const prog = new Map<string, number>();
    const rows = list.map((r) => {
      const key = (r.email || "").toLowerCase();
      const prev = prog.get(key) || 0;
      const isCountable =
        r.status === "confirmed" || r.status === "noshow";
...
function emailHeader(logoUrl: string) {
  ...
  if (banner) {
    // Volles Banner (funktioniert in allen Clients am stabilsten über Tabelle)
    return `
  ...
  // Fallback: weicher Verlauf und Logo mittig
  return `
    <div style="max-width:640px;margin:0 auto 10px auto;padding:28px 0;background:
      radial-gradient(ellipse at center, rgba(179,130,47,0.18) 0%, rgba(179,130,47,0.08) 40%, rgba(255,255,255,0) 72%); text-align:center;">
...
function nextMilestoneTeaser(visitCount: number): string {
  if (visitCount === 4)
    return teaserBox(
      "Heads-up",
      "On your next visit you’ll receive a <b>5% loyalty thank-you</b>.",
    );
  if (visitCount === 9)
    return teaserBox(
      "Almost there",
      "On your next visit you’ll receive a <b>10% loyalty thank-you</b>.",
    );
  if (visitCount === 14)
    return teaserBox(
      "Big milestone ahead",
      "From your next visit you’ll receive a <b>15% loyalty thank-you</b>.",
    );
  return "";
}
