...
// Admin-Mailadresse robust finden
const ADMIN_TO =
...
app.get("/api/config", (_req, res) => {
  res.json({
    ...
    mapsUrl: process.env.VENUE_MAPS_URL || "https://maps.google.com/?q=Moo+4+Lamai+Beach,+84310+Suratthani,+Thailand"
  });
});
...
app.post("/api/reservations", async (req, res) => {
  ...
  // keine Browser-Alerts mehr – Frontend zeigt nun Inline-Toast
...
app.get("/api/admin/reservations", async (req, res) => {
  const date = normalizeYmd(String(req.query.date || ""));
  const view = String(req.query.view || "week"); // "day" | "week" | "month"
...
// Dangerous admin reset – optional (nur mit Key)
app.post("/api/admin/reset", async (req, res) => {
  const key = String(req.body.key || "");
  const expect = String(process.env.ADMIN_RESET_KEY || "");
  if (!expect || key !== expect) return res.status(403).json({ error: "Forbidden" });
...
app.get("/api/export", async (req, res) => {
  try {
    const period = String(req.query.period || "weekly"); // daily|weekly|monthly|yearly
    const date = normalizeYmd(String(req.query.date || ""));
...
// -------- Confirmation E-Mail (angepasst bei 1. Besuch) --------
function confirmationHtml(
  ...
  const intro =
    visitCount <= 1
      ? `<p style="font-size:16px;margin:0 0 12px 0;">Thank you for choosing <b>${site}</b>. We’re excited to host you.</p>`
