...
// ACHTUNG: wir nutzen die MailerSend-HTTP-API, kein Nodemailer/SMTP
...
app.get("/api/health", async (_req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, mail: ok });
  } catch (e: any) {
    res.json({ ok: true, mail: false, error: String(e?.message || e) });
  }
});

// GET-Test für Browser (einfach)
app.get("/api/test-mail", async (req: Request, res: Response) => {
  try {
    const to = String(
      req.query.to ||
        process.env.VENUE_EMAIL ||
        process.env.SMTP_USER ||
        "noxamasamui@gmail.com"
    );
    await sendEmail({
      to,
      subject: "NOXAMA mail test",
      html: "<p>Mail ok.</p>",
      text: "Mail ok.",
...
// Debug-Endpoint: zeigt, ob API-Mode aktiv ist
app.get("/api/debug-mailer", (_req: Request, res: Response) => {
  res.json({
    usesMailerSendApi: !!process.env.MAILERSEND_API_TOKEN,
    from: fromAddress(),
    baseUrl: BASE_URL,
  });
});
...
app.get("/api/slots", async (req: Request, res: Response) => {
  ...
  for (const t of times) {
    ...
    const canReserve =
      sums.reserved < MAX_SEATS_RESERVABLE &&
      sums.total < MAX_SEATS_TOTAL;
    out.push({
      time: t,
      allowed: true,
      reason: null,
...
app.post("/api/reservations", async (req: Request, res: Response) => {
  ...
  try {
    await sendEmail({
      to: created.email,
      subject: `${process.env.BRAND_NAME || "NOXAMA SAMUI"} — Reservation`,
      html,
      idempotencyKey: `res-${created.id}`,
    });
  } catch (e) {
...
app.post("/api/walkin", async (req: Request, res: Response) => {
  ...
  if (!allow.ok)
    return res.status(400).json({
      error: allow.reason === "Blockiert" ? "An diesem Tag ist geschlossen." : "Slot nicht verfügbar.",
    });
...
function confirmationHtml(
  ...
  const site = process.env.BRAND_NAME || "NOXAMA SAMUI";
  ...
  for (const r of list) {
    ...
    const html = `
      <div style="font-family: Georgia, 'Times New Roman', serif; color:#3a2f28;">
        ...
        <p>See you soon,<br/>${process.env.BRAND_NAME || "NOXAMA SAMUI"}</p>
      </div>`;
    try {
      await sendEmail({
        to: r.email,
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  console.log(
    "MAILER MODE:",
    process.env.MAILERSEND_API_TOKEN ? "API" : "SMTP(should be off)"
  );
});
