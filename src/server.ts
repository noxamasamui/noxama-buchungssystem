...
async function overlapping(dateYmd: string, start: Date, end: Date) {
  return prisma.reservation.findMany({
    where: { date: dateYmd, status: { in: ["confirmed","noshow"] },
...
app.post("/api/reservations", async (req: Request,res: Response)=>{
  ...
  try { await mailer().sendMail({ from: fromAddress(), to: created.email, subject: `${process.env.BRAND_NAME || "NOXAMA SAMUI"} — Reservation`, html }); }
...
async function sendReminders() {
  ...
  for (const r of list) {
    ...
    try {
      await mailer().sendMail({ from: fromAddress(), to: r.email, subject: "Reservation reminder", html });
