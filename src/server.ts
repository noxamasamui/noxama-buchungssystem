  ...
  try{
    ...
    if (id){
      rec = await prisma.notice.update({
        where:{ id:String(id) },
        data: { startDate:s, endDate:e, title:String(title), message:String(message),
                requireAck: !!requireAck, active: !!active }
      });
    } else {
      rec = await prisma.notice.create({
        data: { startDate:s, endDate:e, title:String(title), message:String(message),
                requireAck: !!requireAck, active: active==null ? true : !!active }
      });
    }
    res.json(rec);
  }catch(e){
