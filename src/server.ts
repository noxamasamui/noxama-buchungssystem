...
async function overlapping(dateYmd: string, start: Date, end: Date) {
  return prisma.reservation.findMany({
    where: {
      date: dateYmd,
      status: { in: ["confirmed", "noshow"] }, // exakt wie im Schema
...
app.post("/api/admin/closure", async (req: Request,res: Response)=>{
  try{
    ...
    const c = await prisma.closure.create({ data: { id: nanoid(12), startTs:s, endTs:e, reason:String(reason || "Closed") } });
    res.json(c);
  }catch(err){
...
app.post("/api/admin/closure/day", async (req: Request,res: Response)=>{
  try{
    ...
    const c = await prisma.closure.create({ data: { id: nanoid(12), startTs:s, endTs:e, reason } });
    res.json(c);
  }catch(err){
