...
// ⬇️ WICHTIG: nur noch API, kein SMTP
...
// wenn vorhanden, nimm PUBLIC_BASE_URL (Render Env); sonst Fallback
const BASE_URL =
...
app.get("/api/health", async (_req: Request, res: Response) => {
  try {
    const ok = await verifyMailer(); // prüft API-Key / Transport
  ...
  if (!allow.ok) {
    const msg =
      allow.reason === "Blockiert"
        ...
        : allow.reason === "Nach Ladenschluss" ||
          allow.reason === "Vor Öffnung"
