...
const MAX_SEATS_RESERVABLE = num(process.env.ONLINE_SEATS_CAP, 40);
const MAX_ONLINE_GUESTS = num(process.env.MAX_ONLINE_GUESTS, 10);
const WALKIN_BUFFER = num(process.env.MAIL_POOL, 0) ? 8 : 8; // Platzhalter, gleiche Logik wie zuvor
...
function normalizeYmd(input: string): string {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;                 // 2025-11-19
  if (/^\d{1,2}\.\d{1,2}\.\d{4}$/.test(s)) {                   // 19.11.2025
...
/* ───── Loyalty (wie zuvor) ───── */
function loyaltyDiscountFor(visit: number): number { if (visit >= 15) return 15; if (visit >= 10) return 10; if (visit >= 5) return 5; return 0; }
function loyaltyTeaseNext(visit: number): 0|5|10|15 { if (visit === 4) return 5; if (visit === 9) return 10; if (visit === 14) return 15; return 0; }
function loyaltyUnlockedNow(visit: number): 0|5|10|15 { if (visit === 5) return 5; if (visit === 10) return 10; if (visit === 15) return 15; return 0; }
function ordinal(n:number){ const v=n%100; if(v>=11&&v<=13) return `${n}th`; const u=n%10; if(u===1) return `${n}st`; if(u===2) return `${n}nd`; if(u===3) return `${n}rd`; return `${n}th`; }
  ...
  if (MAIL_HEADER_URL) {
    return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td align="center" style="padding:0">
        <img src="${MAIL_HEADER_URL}" alt="${BRAND_NAME}" style="display:block;width:100%;max-width:640px;height:auto;border:0;outline:none;">
      </td></tr></table>`;
  }
  return `<div style="max-width:640px;margin:0 auto 10px auto;padding:28px 0;background:
...
function confirmationHtml(p:{firstName:string;name:string;date:string;time:string;guests:number;cancelUrl:string;visitNo:number;currentDiscount:number;}){
  const header=emailHeader(MAIL_LOGO_URL);
  const tease=loyaltyTeaseNext(p.visitNo);
  const teaser = tease ? `<div style="margin:16px 0;padding:12px 14px;background:#eef7ff;border:1px solid #cfe3ff;border-radius:10px;text-align:center;">
  ...
  return `<div style="font-family:Georgia,'Times New Roman',serif;background:#fff8f0;color:#3a2f28;padding:24px;border-radius:12px;max-width:640px;margin:auto;border:1px solid #e0d7c5;">
    ${header}
...
function canceledGuestHtml(p:{firstName:string;name:string;date:string;time:string;guests:number;rebookUrl:string;}){
  const header=emailHeader(MAIL_LOGO_URL);
  return `<div style="font-family:Georgia,serif;background:#fff8f0;color:#3a2f28;padding:24px;border-radius:12px;max-width:640px;margin:auto;border:1px solid #e0d7c5;">
    ${header}
...
app.get("/api/slots", async (req,res)=>{
  ...
  if (!anyOpen && out.length > 0) {
    ...
    else out.forEach(s => { if (!s.reason) s.reason = "Fully booked for this date. Please choose another day."; });
  }
  ...
  try{
    ...
    const created = await prisma.reservation.create({
      data: {
        ...
        firstName, name, email, phone: String(phone || ""),
    ...
    const html = confirmationHtml({
      firstName: created.firstName, name: created.name, date: created.date, time: created.time,
      guests: created.guests, cancelUrl, visitNo, currentDiscount,
    });
    try { await sendMail(created.email, `${BRAND_NAME} — Reservation`, html); } catch {}
    ...
    res.json({ ok:true, reservation: created, visitNo, discount: currentDiscount, nowUnlockedTier: unlocked, nextMilestone: teaseNext });
  }catch(err){
  ...
  if (r.status !== "canceled") {
    ...
    if (r.email && r.email !== "walkin@noxama.local") {
      const gHtml = canceledGuestHtml({
        ...
        guests: r.guests, rebookUrl: `${BASE_URL}/`,
      });
...
app.get("/api/admin/reservations", async (req,res)=>{
  ...
  if (view === "week" && date) {
    ...
    list = await prisma.reservation.findMany({ where: { startTs: { gte: from, lt: to } }, orderBy: [{ startTs:"asc" }, { date:"asc" }, { time:"asc" }] });
...
app.post("/api/admin/closure", async (req,res)=>{
  try{
    const startTs = new Date(String(req.body.startTs).replace(" ", "T"));
    const endTs   = new Date(String(req.body.endTs).replace(" ", "T"));
    const reason  = String(req.body.reason || "Closed");
    if (isNaN(startTs.getTime()) || isNaN(endTs.getTime()) || endTs <= startTs)
      return res.status(400).json({ error: "Invalid range" });
    const c = await prisma.closure.create({ data: { startTs, endTs, reason } });
    res.json(c);
  }catch(err){
...
app.get("/api/admin/notice", async (req,res)=>{
  try{
    const date = normalizeYmd(String(req.query.date || ""));
...
app.get("/api/notice", async (req,res)=>{
  try{
    ...
    if (!date) return res.json(null);
    const n = await prisma.notice.findFirst({ where: { date } });
    res.json(n ? { message: n.message } : null);
  }catch(err){
    console.error("public notice error:", err);
    res.status(500).json({ error:"Failed to load notice" });
  }
});
  ...
  try{
    ...
    for(const r of list){
      const html = confirmationHtml({
        ...
        cancelUrl: `${BASE_URL}/cancel/${r.cancelToken}`, visitNo: 0, currentDiscount: 0
      });
      try{
