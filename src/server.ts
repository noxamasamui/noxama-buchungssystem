...
app.post("/api/reservations", async (req,res)=>{
  ...
  // Besuchszahl dieses Gasts (nur confirmed/noshow, inkl. aktueller)
  const visitCount = await prisma.reservation.count({ where:{ email: created.email, status:{ in:["confirmed","noshow"] } } });
  const discount = discountForVisit(visitCount); // 5%: 5–9, 10%: 10–14, 15%: 15+

  const cancelUrl = `${BASE_URL}/cancel/${token}`;
  const html = confirmationHtml(created.firstName, created.name, created.date, created.time, created.guests, cancelUrl, visitCount, discount);
...
app.get("/api/export", async (req,res)=>{
  try{
    ...
    // progressive visit count per email (only confirmed/noshow)
    const prog = new Map<string,number>();
    const rows = list.map(r=>{
      const key=(r.email||"").toLowerCase();
      const prev = prog.get(key) || 0;
      const countable = r.status==="confirmed" || r.status==="noshow";
      const nowCount = countable ? prev+1 : prev;
...
function teaserBox(title:string, line:string){
  return `<div style="margin:16px 0;padding:12px 14px;background:#eef7ff;border:1px solid #cfe3ff;border-radius:10px;text-align:center;">
    <div style="font-size:18px;margin-bottom:6px;">${title} ✨</div>
    <div style="font-size:15px;">${line}</div>
  </div>`;
}
function nextMilestoneTeaser(visitCount:number){
